...
import type { OneDriveStatus, ProcessingJob, ProviderStatus, ResourceItem, UploadItem } from "./types";
...
export default function MaintenanceCenter({ oneDrive, aiConfigured, providers, jobs, uploads, resources, onReload, onNotice, onExport }: Props) {
  ...
  async function processFile(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const uploaded = await jsonRequest<{ id: number; status: string }>("/api/uploads", { method: "POST", body: uploadBody });
      const result = await jsonRequest<{ oneDriveSynced: boolean; aiEnhanced: boolean }>("/api/processing", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ inputType: "upload", uploadId: uploaded.id, category: fileCategory, deleteOriginalOnSuccess: true }) });
      form.reset();
      await onReload();
      onNotice(`文件已生成Markdown${result.oneDriveSynced ? "；原文件已进入OneDrive回收站，30天后自动清理" : "；原文件暂时保留，等待OneDrive同步"}`);
    } catch (error) { await onReload(); onNotice((error as Error).message); } finally { setProcessing(false); }
  }
  ...
  return <section>
    ...
    <section className="panel provider-status-panel"><div className="panel-heading"><div><p className="eyebrow">PROVIDER STATUS</p><h2>能力接口状态</h2><p>本轮只显示配置状态，不安装或模拟 OCR、STT 与口语评估模型。</p></div></div><div className="provider-status-list">{providers.map((provider) => <article className={provider.configured ? "configured" : ""} key={provider.id}><span>{provider.id === "ai" ? "AI" : provider.id === "ocr" ? "OCR" : provider.id === "stt" ? "STT" : "PR"}</span><div><strong>{provider.label}</strong><small>{provider.provider}</small></div><em>{provider.configured ? "已配置" : "未配置"}</em></article>)}</div></section>
    ...
    <section className="panel jobs-panel"><div className="panel-heading"><div><p className="eyebrow">PROCESSING QUEUE</p><h2>处理记录</h2><p>失败的任务不会删除原文件。</p></div><span className="count-badge">{jobs.length} 项</span></div><div className="job-list">{jobs.map((job) => <article key={job.id}><span className={`job-state ${job.status}`}>{job.status === "complete" ? "✓" : job.status === "failed" ? "!" : "…"}</span><div><strong>{job.sourceName}</strong><small>{job.stage}{job.error ? ` · ${job.error}` : ""}</small><i><b style={{ width: `${job.progress}%` }} /></i></div><em>{dateText(job.createdAt)}</em></article>)}{!jobs.length && <div className="empty-state small">提交文件或网页后，处理记录会显示在这里。</div>}</div></section>
    <section className="panel batch-panel"><div className="panel-heading"><div><p className="eyebrow">BATCH MAINTENANCE</p><h2>资源库批量维护</h2><p>学习工具网站不在这里出现；这里只维护资料库内容。</p></div><span className="count-badge">已选 {selectedIds.length}</span></div><div className="batch-toolbar"><input value={batchCategory} onChange={(event) => setBatchCategory(event.target.value)} placeholder="新的分类名称" /><button className="button secondary" onClick={() => void batchAction("category")}>批量改分类</button><button className="button secondary" onClick={() => void batchAction("hide")}>批量隐藏</button><button className="button danger" onClick={() => void batchAction("delete")}>批量删除</button></div><div className="batch-list"><label className="batch-all"><input type="checkbox" checked={Boolean(library.length) && selectedIds.length === library.length} onChange={(event) => setSelectedIds(event.target.checked ? library.map((item) => item.id) : [])} /> 全选当前 {library.length} 项</label>{library.slice(0, 150).map((item) => <label key={item.id}><input type="checkbox" checked={selectedIds.includes(item.id)} onChange={(event) => setSelectedIds((current) => event.target.checked ? [...current, item.id] : current.filter((id) => id !== item.id))} /><span><strong>{item.title}</strong><small>{item.category} · {item.processingStatus} · {item.markdownPath || "未生成Markdown"}</small></span></label>)}</div></section>

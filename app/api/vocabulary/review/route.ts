...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json() as { id?: number; rating?: number };
    ...
    await getDatabase().batch([
      getDatabase().prepare(`UPDATE vocabulary SET mastered=0,review_count=?,next_review_at=?,fsrs_state=?,fsrs_stability=?,fsrs_difficulty=?,fsrs_elapsed_days=?,fsrs_scheduled_days=?,fsrs_learning_steps=?,fsrs_reps=?,fsrs_lapses=?,fsrs_last_review_at=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND owner_id=?`)
        .bind(card.reps, card.due.toISOString(), card.state, card.stability, card.difficulty, card.elapsed_days, card.scheduled_days, card.learning_steps, card.reps, card.lapses, card.last_review?.toISOString() || now.toISOString(), body.id, ownerId),

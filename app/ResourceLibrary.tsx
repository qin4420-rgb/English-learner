...
import type { ProcessingJob, ReadingFolderItem, ReadingProgressItem, ResourceItem } from "./types";
...
export default function ResourceLibrary({ resources, jobs, onRead, onStartLearning, onOpenMaintenance, onReloadResources, onNotice, onToggleFavorite }: Props) {
  ...
  const smartCounts = useMemo(() => Object.fromEntries(SMART_VIEWS.map((view) => [view.id, library.filter((item) => isInSmartView(item, view.id, progress[item.id]?.progressRatio || 0)).length])) as Record<SmartView, number>, [library, progress]);
  const filtered = useMemo(() => library.filter((item) => {
    if (!isInSmartView(item, smartView, progress[item.id]?.progressRatio || 0)) return false;
  ...
  }), [dateFilter, deferredSearch, folderId, folders, learningUse, library, progress, smartView, sort, source, status, tag, type]);
  ...
  async function reprocessResource(resource: ResourceItem) {
    ...
    try {
      const result = await jsonRequest<{ jobId: number }>("/api/processing", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ resourceId: resource.id }) });
  ...
  const mediaUrl = quickResource ? quickMetadata?.podcast?.audioUrl || quickResource.sourceUrl || quickResource.url : "";

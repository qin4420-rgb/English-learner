...
import type { ReadingFolderItem, ResourceItem } from "./types";
...
export default function ResourceLibrary({ resources, onRead, onMaintain, onReloadResources, onNotice, onToggleFavorite }: Props) {
  ...
  useEffect(() => {
    jsonRequest<{ folders: ReadingFolderItem[] }>("/api/reading-folders")
      .then((data) => setFolders(data.folders))
      .catch((error: Error) => onNotice(error.message));
  }, [onNotice]);

  async function assignReadingFolder(resourceId: number, folderId: number | null) {
    try {
      ...
      onNotice(folderId ? "文章已加入精读文件夹" : "文章已移到精读书架的未分类区");
    } catch (error) {

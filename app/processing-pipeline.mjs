...
export function mapProcessingError(error, context = {}) {
  ...
  const catalog = {
    ...
    STT_TIMEOUT: ["语音识别服务超时。原媒体和之前步骤仍然保留。", true, ["重试STT", "稍后继续"], "failed"],
    MEDIA_SOURCE_RESTRICTED: ["泛听可用，但当前媒体来源不允许建立精听版本。", false, ["上传合法音频", "添加字幕", "打开原始来源"], "needs_action"],
    SUBTITLE_QUALITY: ["字幕已读取，但时间轴或内容存在异常。", true, ["复核字幕", "改用STT重新生成"], "needs_action"],
